...
%length(tauS)
%%%bucle en tauS o taun segun cual sea el dat X!
for i=1:length(tauN)%%%%!!!!!La version original no funciona con dMo vector y da error en fitUsadel. Hay que vectorizarla! esto es chapucero!
    %f2(i,:)=psi(1/2+(tauN+tauS(i))./(2*pi*T*Kb*(tauN.*tauS(i))))-(1+tauS(i)./tauN).*log(Tc0./T);
    f2(i,:)=psi(1/2+(tauN(i)+tauS)./(2*pi*T*Kb*(tauN(i).*tauS)))-(1+tauS./tauN(i)).*log(Tc0./T);
